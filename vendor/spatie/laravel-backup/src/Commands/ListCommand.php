...
class ListCommand extends BaseCommand
{
     ...
     * @var string
     */
    protected $signature = '备份：列表';

    /**
     * @var string
     */
    protected $description = '显示所有备份的列表。';
    ...
    protected function displayConnectionErrors(Collection $backupDestinationStatuses)
    {
        ...
        $this->warn('');
        $this->warn('无法访问的备份目的地');
        $this->warn('-------------------------------');

        $unreachableBackupDestinationStatuses->each(function (BackupDestinationStatus $backupStatus) {
            $this->warn("无法达到备份 {$backupStatus->getBackupName()} on disk {$backupStatus->getFilesystemName()} because:");
